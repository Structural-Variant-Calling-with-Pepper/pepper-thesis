        ...
        switch (cigar.operation) {
            ...
            case CIGAR_OPERATIONS::DEL:
                ...
                if (ref_position -1 >= ref_start && ref_position - 1 <= ref_end) {
                    char ref_base = reference_sequence[ref_position - 1 - ref_start];
                    int base_index = (int)(ref_position - 1 - ref_start + cumulative_observed_insert[ref_position - 1 - ref_start]);
                    int ref_end_index = ref_position - ref_start + cigar.length - 1;
                    char ref_end_base = reference_sequence[ref_end_index];
                    int base_end_index = (int)(ref_end_index + cumulative_observed_insert[ref_end_index]);
                    ...
                    if (print_colored_debug) {
                        // cerr << HCYN << "ref_index: " << ref_position - 1 - ref_start << " ref_end_index: " << ref_end_index << endl;
                    ...
                    if(candidate_string.length() >= candidate_length_thresh) {
                        ...
                        if (print_colored_debug) {
                            cerr << BOLDWHITE <<  "Deletion end base: " << ref_position - ref_start + cigar.length << RESET <<endl;
                        }
                        delete_count[ref_position - ref_start + cigar.length - 1] += 1;
                        int region_index = (int) (ref_position - 1 - ref_start);
                        int region_end_index = (int) (ref_position - ref_start + cigar.length - 1);
                        
                        if (fahmid_check) {
                            cerr << "Fahmid Check Deletion region: [" << region_index << ", " << region_end_index << "]" << endl;
                        }
...
vector<CandidateImageSummary> RegionalSummaryGenerator::generate_summary(vector <type_read> &reads,
    ...
    for(int i=0;i<region_size;i++){
        ...
        bool at_least_one_fraction_positive = (snp_fraction >0 || insert_fraction >0 || delete_fraction >0);
...
//            cout<<candidate_string<<" "<<allele_depth<<" "<<candidate_frequency<<endl;
            ...
            if (train_mode) {
                ...
                if(hp1_truths.size()==0 && hp2_truths.size()==0 && isIndel) {
                    ...
                    fixed_base_index = -1;
                    for (int i = max(0, base_index - 200); i < min(base_index + 200, region_size); i++) {
                        ...
                        bool within_10pc_hap1 = candidate_string.length() >= 0.9 * hap1_max_len && candidate_string.length() <= 1.1 * hap1_max_len;
                        bool within_10pc_hap2 = candidate_string.length() >= 0.9 * hap2_max_len && candidate_string.length() <= 1.1 * hap2_max_len;
