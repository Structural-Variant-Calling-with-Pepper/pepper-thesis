...
//    stop += 0;
    ...
    while(sam_itr_next(this->hts_file, iter, alignment) >= 0) {
        ...
        for (int k = 0; k < alignment->core.n_cigar; k++) {
            ...
            bool debug = query_name== "m64012_190921_234837/176095316/ccs" && false;
            if (debug) {
            ...
            switch (cigar_op) {
                ...
                case BAM_CSOFT_CLIP:
                    ...
                    if (modified_cigar_length > 0) {
                        ...
                        if(cigar_op == BAM_CSOFT_CLIP && print_colored_debug) cerr << "Saving soft clip: " << modified_cigar_length << endl;
